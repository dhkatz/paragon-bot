...
class EmbedPages:
    ...
    def __init__(self, ctx, *, entries, icon_url=None, show_entry_count=True):
        ...
        self.maximum_pages = len(self.entries)
        self.show_entry_count = show_entry_count
    ...
    async def show_page(self, page, *, first=False):
        self.current_page = page
        self.embed.clear_fields()
        embed = self.get_page(page)
        if not isinstance(embed, discord.Embed):
            raise CannotPaginate('Invalid objects passed!')

        self.embed.title = embed.title
        self.embed.colour = embed.colour

        if not isinstance(embed.thumbnail.url, _EmptyEmbed) and embed.thumbnail.url != '':
            self.embed.set_thumbnail(url=embed.thumbnail.url)

        if not isinstance(embed.url, _EmptyEmbed) and embed.url != '':
